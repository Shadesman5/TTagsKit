...
<div id="menuList" class="liste uk-switcher uk-margin">
    <div :class="settings.categoryContainer || defaults.categoryContainer" v-for="category in list.categories | orderBy 'position'" data-type="Category" data-id="{{category.id}}">
        ...
        <ul class="uk-list uk-flex uk-flex-column" data-type="Category Items">
            <li :class="settings.itemContainer || defaults.itemContainer" v-for="item in category.items | orderBy 'position'" data-type="Item" data-id="{{item.id}}" data-uk-grid-margin>
                ...
                <div v-if="item.allergens.length" :class="settings.itemAllergensContainer || defaults.itemAllergensContainer" data-type="Item Allergens">
                    <a href="#{{item.id}}" data-uk-modal>
                        ...
                        <img v-for="allergen in item.allergens | orderBy 'id'" :class="settings.itemAllergenImage || defaults.itemAllergenImage" :title="allergen.title" :src="allergen.image" :alt="allergen.title" data-type="Allergen" />
                    </a>
                    <div id="{{item.id}}" class="uk-modal">
                        <div class="uk-modal-dialog">
                            <a href="" class="uk-modal-close uk-close uk-close-alt"></a>
                            <div class="uk-modal-header">
                                <h3>{{ 'Allergens from' | trans }} {{ item.title }}:</h3>
                            </div>
                            <div class="uk-grid uk-grid-small uk-flex-center uk-text-center uk-grid-width-small-1-2 uk-grid-width-medium-1-3" data-uk-grid-match="{target: '.modalAllergen'}" data-uk-grid-margin>
                                <div v-for="allergen in item.allergens | orderBy 'id'">
                                    <div :class="settings.itemModalAllergen || defaults.itemModalAllergen">
                                        <img :title="allergen.title" :src="allergen.image" width="50px" :alt="allergen.title" data-type="Allergen" />
                                        <div class="uk-thumbnail-caption">
                                            <p class="allergenTitle uk-text-bold uk-margin-xsmall-bottom">{{ allergen.title }}:</p>
                            ...
                            <hr>
                            <div class="uk-modal-header">
                                <h3>{{ 'Additives from' | trans }} {{ item.title }}:</h3>
                            </div>
                            <div class="uk-grid uk-grid-small uk-flex-center uk-text-center uk-grid-width-small-1-2 uk-grid-width-medium-1-3" data-uk-grid-match="{target: '.modalAllergen'}" data-uk-grid-margin>
                                <div v-for="allergen in item.allergens | orderBy 'id'">
                                    <div :class="settings.itemModalAllergen || defaults.itemModalAllergen">
