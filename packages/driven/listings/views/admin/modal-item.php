<v-modal v-ref:itemmodal large>
    ...
    <form class="uk-form" v-validator="item_form" @submit.prevent="saveItem(category_model.id) | valid">
        ...
        <div class="item-allergens uk-form-row uk-accordion" data-uk-accordion="{showfirst: true}">
            <section class="uk-panel-box uk-padding-remove">
                <h3 class="uk-form-label uk-text-center uk-margin-remove uk-accordion-title">
                    {{ 'Allergen List' | trans }}<br>
                    <small>{{ 'The 14 main allergen groups' | trans }}</small>
                </h3>

                <div class="uk-grid uk-grid-small uk-flex-center uk-accordion-content uk-margin-top" v-cloak>
                    <label v-for="allergen in allergens | orderBy 'position'" class="allergene uk-width uk-flex uk-flex-center uk-flex-middle pk-table-max-width-150 uk-margin-bottom">
                        <input class="uk-hidden" type="checkbox" :value="allergen.id" v-model="selectedAllergenIds">
                        <img class="uk-panel uk-border-rounded uk-link" :src="'/' + allergen.image"  width="100%" :alt="allergen.title">
                    </label>
                </div>
            </section>
        </div>
